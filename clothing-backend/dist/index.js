...
app.get('/users/telegram/:telegramId', (req, res) => {
    ...
    const stmt = db_1.default.prepare('SELECT * FROM users WHERE telegramId = ?');
...
app.put('/users/:id', upload.single('photo'), (req, res) => {
    ...
    const selectStmt = db_1.default.prepare('SELECT * FROM users WHERE id = ?');
    ...
    if (req.file) {
        if (photoUrl) {
            const oldPath = path_1.default.join(__dirname, '..', photoUrl);
    ...
    const updateStmt = db_1.default.prepare('UPDATE users SET firstName = ?, lastName = ?, username = ?, photoUrl = ? WHERE id = ?');
...
app.get('/outfits', (req, res) => {
    ...
    if (userId) {
        ...
        stmt = db_1.default.prepare('SELECT * FROM outfits ORDER BY createdAt DESC');
